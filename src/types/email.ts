...
// 条件逻辑：运算符
export type ConditionOperator = 'eq' | 'neq' | 'isEmpty' | 'isNotEmpty';

// 条件逻辑：单条件（一期支持等于/不等于/为空/不为空）
export interface SimpleCondition {
  /** 判断的变量 key，如 "user.tier" */
  variableKey: string;
  operator: ConditionOperator;
  /** eq / neq 时必填；isEmpty / isNotEmpty 时留空 */
  value?: string;
}
...
export interface LoopBinding {
   ...
   * 默认 0（展示第一项）。
   * horizontal 模式下预览会展示所有可用项，此值不影响预览。
   */
  previewIndex?: number;
  /**
   * 展开方向（默认 'vertical'）：
   * - 'vertical'：N 项纵向堆叠为 N 行
   * - 'horizontal'：N 项并排为 N 个等宽列，包裹在横向父容器中
   */
