 ...
 *  4. inline SVG → 轉 <img src="data:image/svg+xml;...">（Gmail 不支持 inline SVG）
 *  5. 图片加 HTML width/height 属性
 *  6. 替换 CSS 变量（var(--xxx)）、fit-content 等不相容写法
 *  7. flex/grid 布局轉 HTML table（邮件客户端不支持 flex/grid）
 *  8. 强制垂直堆叠 + 剩余 flex 轉 block
 *
 * Gmail 不显示任何 SVG（含 data:image/svg+xml 与 .svg 链接），需使用 prepareEmailHtmlAsync
 * 在导出时将 SVG 轉为 PNG，发送邮件时请用异步版本。
 */
...
/** 发送邮件时使用：与 prepareEmailHtml 相同，但会将所有 SVG 图片轉为 PNG，確保 Gmail 可显示。 */
...
/** 用双层 table 嵌套将整封邮件在客户端視窗中居中（业界標準做法） */
...
/** 高清倍率：以 3x 渲染確保在 Retina 屏与邮件客户端中清晰 */
...
/**
 * 与编辑器 ImageBlock 语义一致：容器高度由图片決定，overlay 不撑高。
...
function transformImageLayoutMode(root: HTMLElement) {
  root.querySelectorAll('[class*="layoutContainer"]').forEach((container) => {
    ...
    if (isFixedSizeImage) {
      // 固定尺寸图片：保留原始寬高，匹配编辑器中 object-fit: fill 的拉伸行为
      ...
      el.style.width = '100%';
      // 高度始終由图片比例決定（minHeight），保底避免容器塌陷。
      // 不再依赖叠加层高度（旧的 isFillLayoutMode 双行为已移除）。
    ...
    // 提前移除 flex，手动轉为 block + text-align（防止被全局循环覆盖）
    ...
    // 垂直对齐：容器高度由图片比例決定（minHeight），覆盖层相對定位后，仅在空状态时補 padding 近似居中
...
function fixImages(root: HTMLElement) {
  root.querySelectorAll('img').forEach((img) => {
    // 加 HTML width/height 属性（email client 依赖這些）
    const naturalW = img.naturalWidth;
    const naturalH = img.naturalHeight;
...
function fixCssCompatibility(root: HTMLElement) {
  root.querySelectorAll('*').forEach((el) => {
    ...
    // 替换 fit-content：改为 auto + inline-block（等價收缩宽度，兼容 Gmail）
    if (style.width === 'fit-content') {
      style.width = 'auto';
      // 只在元素原本未指定 display 或为 block 时改为 inline-block
      // 以保留「收缩宽度」语义，避免被拉成整行导致移動端左偏。
    ...
    // 注意：Chrome 在 computedStyle 会展开 inset，但 DOM style 可能還是 inset
    // cloneNode 后 style 应已展开为 top/right/bottom/left
...
/* ====== 7. 将 flex/grid 布局轉为 HTML table（邮件客户端不支持 flex/grid） ====== */
...
/** 将水平 flex 容器轉为 <table><tr><td>…</td>…</tr></table> */
function flexRowToTable(container: HTMLElement) {
  ...
  // 如果原始容器是 auto 寬（fit-content 已被轉成 auto），table 不應撑滿 100%
...
/** 将 CSS Grid 容器轉为 <table>（按 columnsPerRow 分行） */
...
/** 建立自適應宽度的 table（width: auto，可被父級居中） */
 ...
 * 宽度、对齐等样式必须从 ComponentWrapper（topLevelWrapper）读取，不可从 blockContent 推斷。
 * 若画布为 contentDistribution=spaceBetween，移除 flex 后需用其他方式（如 li 的 margin）还原
 * 「首尾贴边、中間均分」的视觉，避免仅删除 gap/justify-content 导致均分语义丢失。
 */
function ensureVerticalStack(root: HTMLElement) {
  ...
  if (list) {
    ...
    list.querySelectorAll(':scope > li').forEach((li) => {
      ...
      if (topLevelWrapper) {
        ...
        // 读取組件自身的水平对齐（alignItems 控制 flex-direction:column 的水平对齐）
        ...
        if (isFitContent) {
          // fitContent 容器：由「画布对齐」決定 wrapper 在 li 中的位置
          if (canvasHAlign !== 'left') {
            ...
            // 冗餘居中层：li 內包一层 div（避免移動端忽略 li 的 text-align）
            ...
            // wrapper 保持 inline-block + text-align:left（内容不繼承居中）
            wrapper.style.display = 'inline-block';
            wrapper.style.setProperty('text-align', 'left', 'important');
          }
        } else if (hAlign !== 'left') {
          // 全宽容器：由組件自身 contentAlign 決定內部内容的水平对齐
          // 多层冗餘 text-align（wrapper + blockContent + li），不再嵌套 alignment table
          ...
          // 對 wrapper 內 display:block 的直接子元素（非全宽、非 table）轉为 inline-block，
  ...
  // 對所有元素：box-sizing + 移除残余 flex（Gmail 会移除后导致乱排）
  const FLEX_PROPS = [
  ...
  // 收集需要用 table 实现对齐的 flex 容器（从叶到根，避免 replaceWith 干扰遍历）
  ...
  flexElements.reverse().forEach((htmlEl) => {
    ...
    // 這樣父級的 text-align / td align 才能對其生效
    ...
    // 水平对齐：仅對 block（全宽）元素设 text-align
    ...
    // 但對 content-sized（inline-block / fitContent）元素跳過：
    // 否則在 Gmail PC 容易把本該收缩宽度的 overlay 徽章/按鈕拉伸为整行。
